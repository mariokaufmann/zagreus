...
impl ConfigValidate for TextConfig {
    fn validate(&self, validation_data: &ValidationData) -> Result<(), ZagreusError> {
        for text_element_config in &self.elements {
            if !validation_data
                .data_elements
                .has_data_element(&text_element_config.id)
            {
                return Err(ZagreusError::from(format!(
                    "Text config contains unknown element {}.",
                    &text_element_config.id
                )));
            }

            // check alignment
            if text_element_config.align == TextAlignment::Center
                && text_element_config.align_with.is_empty()
            {
                return Err(ZagreusError::from(format!(
                    "Text element {} is center-aligned but no alignWith is configured.",
                    &text_element_config.id
                )));
            }
            ...
            if !text_element_config.align_with.is_empty()
                && !validation_data
                    .data_elements
                    .has_data_element(&text_element_config.align_with)
            {
                return Err(ZagreusError::from(format!(
                    "Text element {} is configured to be aligned with unknown element {}.",
                    &text_element_config.id, &text_element_config.align_with
                )));
            }
        }
        ...
        let text_config = TextConfig {
            elements: vec![
                TextElementConfig {
                    ...
                    align_with: String::from(""),
                },
                TextElementConfig {
                    id: String::from("id2"),
                    align: TextAlignment::Center,
                    align_with: String::from("id1"),
                },
            ],
        };
