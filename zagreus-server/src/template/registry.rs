...
impl TemplateRegistry {
    ...
    pub fn load_templates(&mut self) {
        ...
        match get_templates_data_folder(&self.data_folder) {
            Ok(templates_folder) => match std::fs::read_dir(templates_folder) {
                Ok(read_dir) => {
                    for entry in read_dir {
                        match entry {
                            ...
                            Err(err) => error!(
                                "Error while trying to get directory entry of {}: {}.",
                ...
                Err(err) => {
                    error!(
                        "Could not list directory entries for {}: {}.",
                        self.data_folder.display(),
                        err
                    );
                }
            ...
            Err(err) => error!(
                "Could not get templates folder. Cannot load templates: {}.",
                err
            ),
        }
    }
    ...
    fn load_template_folder(&mut self, template_folder: &Path) {
        if template_folder.is_dir() {
            debug!(
                "Trying to load template from directory {}.",
    ...
    fn load_template(&mut self, template_name: &str) {
        match crate::template::Template::load(&self.data_folder, template_name) {
            Ok(template) => match self.templates.insert(String::from(template_name), template) {
