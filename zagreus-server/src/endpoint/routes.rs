...
pub fn get_routes(server_controller: Arc<ServerController>, ws_server: Arc<WebsocketServer>, template_registry: ServerTemplateRegistry, configuration: &ZagreusServerConfig)
    ...
    let template_param_filter = template_filter
        .and(warp::path::param());
    let data_filter = template_param_filter
        .and(warp::path("data"));
    let text_filter = data_filter
        .and(warp::path("text"))
        .and(websocket_server_data.clone())
        .and(warp::body::json());
    let set_text_filter = text_filter
        .and(warp::post())
        .and_then(|template_name: String, server: Arc<WebsocketServer>, payload: SetTextDto| async move {
            let message = TemplateMessage::SetText { id: &payload.id, text: &payload.text };
            send_template_message(&template_name, server, message).await
        });
    let class_filter = data_filter
        .and(warp::path("class"));
    let add_class_filter = class_filter
        .and(warp::path("add"))
        .and(warp::body::json())
        .and(websocket_server_data.clone())
        .and(warp::post())
        .and_then(|template_name: String, payload: ManipulateClassDto, server: Arc<WebsocketServer>| async move {
            let message = TemplateMessage::AddClass { id: &payload.id, class: &payload.class };
            send_template_message(&template_name, server, message).await
        });
    let remove_class_filter = class_filter
        .and(warp::path("remove"))
        .and(warp::body::json())
        .and(websocket_server_data.clone())
        .and(warp::post())
        .and_then(|template_name: String, payload: ManipulateClassDto, server: Arc<WebsocketServer>| async move {
            let message = TemplateMessage::RemoveClass { id: &payload.id, class: &payload.class };
            send_template_message(&template_name, server, message).await
        });
    let animation_filter = data_filter
        ...
        .and(websocket_server_data.clone())
        .and(warp::path::param());
    let execute_animation_filter = animation_filter
        .and(warp::post())
        .and_then(|template_name: String, server: Arc<WebsocketServer>, animation_name: String| async move {
            let message = TemplateMessage::ExecuteAnimation { animation_sequence: &animation_name };
            send_template_message(&template_name, server, message).await
        });
...
async fn upload_template(template_name: String, template_registry: ServerTemplateRegistry, form: warp::multipart::FormData)
...
async fn send_template_message(template_name: &str, server: Arc<WebsocketServer>, message: TemplateMessage<'_>)
                               -> Result<impl warp::Reply, warp::reject::Rejection> {
    server.send_message_to_template_clients(&template_name, &message).await;
    Ok(warp::reply())
}